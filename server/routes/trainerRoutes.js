...
router.get("/", async (req, res) => {
  ...
  var condition =
    firstName || lastName
      ? {
          firstName: { $regex: new RegExp(firstName, lastName), $options: "i" },
        }
      : {};
  ...
  Trainer.findByIdAndUpdate(id, req.body)
    .then((data) => {
      if (!data) {
        ...
        res.send({
          message: `Trainer ${data.firstName} info updated sucessfully!`,
        });
    })
    .catch((err) => {
      res
  ...
  Trainer.findByIdAndDelete(id)
    .then((data) => {
      if (!data)
        ...
        res.send({ message: `Traner ${data.firstName} deleted successfully!` });
    })
    .catch((err) => {
      res
        .status(500)
