...
router.post("/", async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      return res
        .status(409)
        .send({ message: "Username already exists!", userLoggedIn: true });
    }
  } catch (error) {
  ...
  await User.findById(id)
    .then((data) => {
      if (!data) {
        ...
        res.send({
          message: `User ${data.firstName} found with an ${data.id}`,
        });
      }
    })
    .catch((err) => {
  ...
  await User.findByIdAndUpdate(id, req.body)
    .then((data) => {
      if (!data) {
        res
          .status(400)
          .send({ message: `Cannot update user with an id=${id}` });
  ...
  await User.findByIdAndRemove(id)
    .then((data) => {
      if (!data) {
        res
          .status(400)
          .send({ message: `Cannot delete user with an id of id=${id}` });
