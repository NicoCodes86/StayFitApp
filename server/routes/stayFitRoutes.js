  ...
  await Exercise.findByIdAndUpdate(id, req.body)
    .then((data) => {
      if (!data) {
        res
          .status(400)
          .send({ message: `Cannot update exercise with an id of id=${id}` });
  ...
  await Exercise.findByIdAndRemove(id)
    .then((data) => {
      if (!data) {
        res
          .status(400)
          .send({ message: `Cannot delete exercise with an id of id=${id}` });
      } else
        res.send({ message: `Exercise with an ${id} deleted succesfully!` });
    })
    .catch((err) => {
      res.status(500).send({
